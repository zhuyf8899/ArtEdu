...
function Toast({ notice }) { return notice ? <div className={`toast toast--${notice.tone}`} role={notice.tone === "error" ? "alert" : "status"}>{notice.tone === "error" ? <WarningCircle size={18} weight="bold" /> : <Check size={18} weight="bold" />}<span>{notice.text}</span></div> : null; }
...
export function AdminDashboard({ actor, onBack, onNavigate = () => {}, initialSection = "overview" }) {
  ...
  const [section, setSection] = useState(operatorOnly ? "reviews" : initialSection); const [sidebarOpen, setSidebarOpen] = useState(false); const [users, setUsers] = useState([]); const [reviews, setReviews] = useState([]); const [dashboard, setDashboard] = useState(null); const [quotaUser, setQuotaUser] = useState(null); const [selectedReview, setSelectedReview] = useState(null); const [toast, setToast] = useState(null);
  ...
  const showToast = (message, tone) => { const text = typeof message === "string" ? message : message?.message || "操作失败，请稍后重试"; const inferredTone = tone || (/失败|错误|无法|不能|不存在|未配置|无权|拒绝|请求失败/.test(text) ? "error" : "success"); setToast({ text, tone: inferredTone }); window.setTimeout(() => setToast(null), 3200); };
  useEffect(() => { Promise.allSettled([operatorOnly ? Promise.resolve(null) : getAdminUsers(), getAdminReviews(), operatorOnly ? Promise.resolve(null) : getAdminDashboard()]).then(([userResult, reviewResult, dashboardResult]) => { if (userResult.status === "fulfilled" && userResult.value) setUsers(userResult.value.items ?? []); else if (!operatorOnly) showToast("用户数据加载失败"); if (reviewResult.status === "fulfilled") setReviews(reviewResult.value.items ?? []); else showToast("审核数据加载失败"); if (dashboardResult.status === "fulfilled" && dashboardResult.value) setDashboard(dashboardResult.value); }); }, [operatorOnly]);

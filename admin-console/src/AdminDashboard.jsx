...
import { CaseReviewContent } from "./CaseReviewContent.jsx";
...
function Overview({ users, reviews, dashboard, onNavigate, onEditQuota }) {
  ...
  return <div className="page-content">
    <section className="metrics-grid">
      ...
      <MetricCard label="待审核作品" value={String(dashboard?.pendingReviews ?? reviews.filter((item) => item.status === "pending").length)} change="数据库实时统计" icon={ShieldCheck} />
...
function ReviewDrawer({ review, onClose, onDecision }) {
  ...
  return <div className="drawer-layer" role="presentation"><button className="drawer-scrim" onClick={onClose} aria-label="关闭审核详情" /><aside className="drawer drawer--review" aria-label="作品审核详情"><div className="drawer__header"><div><p>// {review.id}</p><h2>审核作品</h2></div><button className="icon-button" onClick={onClose}><X size={21} weight="bold" /></button></div><div className="review-hero"><div className="review-hero__icon"><FileImage size={54} weight="thin" /></div><div className="review-hero__label"><span>{review.kind}</span><strong>{review.assets} 个作品文件</strong></div>{review.assetLinks?.[0]?.url ? <a className="outline-button" href={review.assetLinks[0].url} target="_blank" rel="noreferrer"><Eye size={17} weight="bold" /> 打开作品文件</a> : <button disabled><Eye size={17} weight="bold" /> 暂无可预览文件</button>}</div><div className="review-title"><span>{review.id}</span><h3>{review.title}</h3><p>{review.author} · {review.department} · {review.submittedAt}</p></div>{review.editorialIssues?.length > 0 && <section className="editorial-checklist"><strong>投稿资料提醒 · {review.editorialIssues.length} 项</strong><ul>{review.editorialIssues.map((issue) => <li key={issue}>{issue}</li>)}</ul><small>仅供审核参考，不会自动修改作品或审核结论。</small></section>}{review.assetLinks?.length > 0 && <div className="review-assets"><span>作品文件</span>{review.assetLinks.map((asset, index) => <a key={asset.id} href={asset.url} target="_blank" rel="noreferrer">{index + 1}. {asset.fileName}</a>)}</div>}<div className="review-facts"><div><span>生成模型</span><strong>{review.model}</strong></div><div><span>资源类型</span><strong>{review.kind}</strong></div><div><span>安全检测</span><strong className={review.machineStatus.includes("复核") ? "warning-text" : "safe-text"}>{review.machineStatus}</strong></div></div><CaseReviewContent workId={review.id} /><div className="prompt-box"><span>// PROMPT 摘要</span><p>{review.prompt}</p></div><label className="review-note"><span>审核备注</span><textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="通过时可选；驳回时建议填写修改原因……" /></label><div className="drawer__actions drawer__actions--split"><button className="reject-button" onClick={() => onDecision(review.id, "rejected", note)}>驳回并退回修改</button><button className="primary-button" onClick={() => onDecision(review.id, "approved", note)}><Check size={18} weight="bold" /> 通过并发布</button></div></aside></div>;
}

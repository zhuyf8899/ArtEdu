...
function ToolDirectoryCard({ tool, canManage, onEdit, onOpenWorkflowBuilder }) {
  ...
  const content = <><span className="tool-directory__icon"><Icon size={21} weight="duotone" /></span><span><b>{tool.name}</b><small>{tool.category} · {tool.featured ? "推荐工具" : external ? "外部工具" : "平台功能"}</small><em>{tool.detail}</em></span><LinkSimple size={17} weight="bold" /></>;
  const isWorkflowBuilder = tool.id === "tool-directory-workflow";
  return <article className="tool-directory__card">{isWorkflowBuilder ? <button type="button" className="tool-directory__launch" onClick={onOpenWorkflowBuilder}>{content}</button> : <a href={tool.href} target={external ? "_blank" : undefined} rel={external ? "noopener noreferrer" : undefined}>{content}</a>}{canManage && <button className="tool-directory__edit" onClick={() => onEdit(tool)} aria-label={`编辑 ${tool.name}`}><PencilSimple size={15} weight="bold" /></button>}</article>;
}

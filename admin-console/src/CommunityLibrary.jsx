...
import { addWorkComment, createWork, getMyWorks, getWork, getWorkflows, getWorks, submitWork, toggleWorkReaction } from "./services/adminApi.js";

const emptyForm = { title: "", summary: "", discipline: "视觉传达", assetUrl: "", tags: "", workflowId: "" };
...
export function CommunityLibrary({ fallbackWorks, onNotice }) {
  ...
  const publish = async (event) => {
    ...
    setLoading(true);
    try {
      const url = new URL(form.assetUrl);
      const fileName = decodeURIComponent(url.pathname.split("/").pop() || "artwork.jpg").slice(0, 255);
      const mimeType = /\.png($|\?)/i.test(form.assetUrl) ? "image/png" : /\.webp($|\?)/i.test(form.assetUrl) ? "image/webp" : "image/jpeg";
      const work = await createWork({
        ...
        assets: [{ externalUrl: form.assetUrl, fileName, mimeType, assetType: "image", altText: form.title }],
      });
      ...
      onNotice(error.message || "资源地址格式不正确");
    } finally {
      setLoading(false);
    }
  };
  ...
  return <>
    ...
    {showForm && <div className="publish-layer"><button className="publish-scrim" onClick={() => setShowForm(false)} aria-label="关闭" /><form className="publish-form" onSubmit={publish}><header><div><span>// SUBMIT YOUR WORK</span><h2>发布作品</h2></div><button type="button" onClick={() => setShowForm(false)}><X size={20} /></button></header><label>作品名称<input required minLength="2" value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} /></label><label>作品说明<textarea required minLength="2" value={form.summary} onChange={(event) => setForm({ ...form, summary: event.target.value })} /></label><div className="publish-form__row"><label>学科分类<input required value={form.discipline} onChange={(event) => setForm({ ...form, discipline: event.target.value })} /></label><label>使用的工作流<select value={form.workflowId} onChange={(event) => setForm({ ...form, workflowId: event.target.value })}><option value="">未使用工作流</option>{workflows.map((workflow) => <option key={workflow.id} value={workflow.id}>{workflow.name}</option>)}</select></label></div><label>作品图片地址<input required type="url" placeholder="https://.../artwork.jpg" value={form.assetUrl} onChange={(event) => setForm({ ...form, assetUrl: event.target.value })} /><small>当前支持 HTTPS 资源链接；对象存储接入后将升级为本地文件上传。</small></label><label>标签<input placeholder="AI 设计，传统纹样，UI 创作" value={form.tags} onChange={(event) => setForm({ ...form, tags: event.target.value })} /></label><button className="publish-submit" disabled={loading}><CheckCircle size={18} weight="fill" /> {loading ? "正在提交…" : "提交管理员审核"}</button></form></div>}
  </>;
}

...
| 方法 | 路径 | 用途 |
| --- | --- | --- |
| GET | `/api/portal/home` | 聚合首页课程、精选工作流、已发布案例，以及当前用户的模型服务和生成额度快照 |
| GET | `/api/portal/search?query=设计&type=all&tag=入门` | 跨教学资源、工作流和案例社区搜索；支持内容类型与标签过滤 |

搜索结果统一返回 `type`、标题、摘要、作者、分类、标签、目标业务路由和更新时间，并附带各内容类型计数及当前结果可用标签。仅检索已发布课程、已发布工作流和已审核通过案例。

首页响应的 `creation` 字段包含 `enabled`、可用模型摘要和当前用户的日/月/并发额度快照。该数据只用于创作前状态提示；服务端在创建任务时仍会在事务和用户级锁中重新检查额度，前端数据不能替代服务端权限判断。

import { ForbiddenException, HttpException, HttpStatus, Inject, Injectable, NotFoundException, ServiceUnavailableException } from "@nestjs/common";
...
export class GenerationService {
  ...
  private async validateReferences(client: PoolClient, actor: Actor, input: CreateGenerationJobInput) {
    ...
    if (input.workflowVersionId) {
      const version = await client.query(`
        ...
        WHERE version.id = $1 AND workflow.status = 'published' AND version.published_at IS NOT NULL
  ...
  async getJob(actor: Actor, jobId: string) {
    ...
    // 生成任务可能包含用户提示词与素材上下文；运营审核应通过审核队列读取必要信息，
    // 不应拥有通用任务查询权限。

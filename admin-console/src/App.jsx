...
function AppContent() {
  ...
  return <UserPortal account={account} section={sectionFromPath(pathname)} searchQuery={query.get("query") ?? ""} learningCourseId={query.get("course") ?? ""} studioWorkflowId={query.get("workflow") ?? ""} studioRunId={query.get("run") ?? ""} creationStartNew={query.get("new") === "1"} creationId={query.get("id") ?? ""} onNavigate={navigate} onSwitchAccount={signOut} onEnterAdmin={() => navigate("/admin")} />;
}

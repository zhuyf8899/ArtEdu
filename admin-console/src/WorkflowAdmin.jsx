...
function FlowNode({ data }) {
  ...
  return <div className="workflow-flow-node" style={{ "--node-color": meta.color }}>
    ...
    <div className="workflow-flow-node__bar"><FlowArrow size={15} weight="bold" /> {meta.title}</div>
    <strong>{data.label || meta.title}</strong>
    <span>{data.description || meta.hint}</span>
    {data.value && <small>{data.value}</small>}

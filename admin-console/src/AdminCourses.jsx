    ...
    <section className="course-admin-grid">
      <article className="table-panel course-admin-list">
        ...
        {courses.map((course) => <div className="course-admin-row" key={course.id}>
          ...
          <label className="course-resource-upload"><input disabled={busy || !["draft", "rejected"].includes(course.status)} type="file" accept="application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.openxmlformats-officedocument.presentationml.presentation,video/mp4,video/webm,.pdf,.docx,.pptx,.mp4,.webm" onChange={(event) => { const file = event.target.files?.[0]; if (file) run(() => uploadCourseResource(course.id, file), "课程资料已安全上传"); event.target.value = ""; }} /><span>上传资料</span></label>

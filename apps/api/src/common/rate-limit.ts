...
/**
 * Local safety valve for unauthenticated and development traffic. Production
 * deployments with multiple API instances should replace this with a shared
 * gateway/Redis limiter so limits are enforced across every instance.
 */
export async function apiRateLimitHook(request: FastifyRequest, reply: FastifyReply) {
  ...
  return reply.code(429).send({ statusCode: 429, message: "请求过于频繁，请稍后再试" });
}

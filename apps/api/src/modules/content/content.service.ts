...
export class ContentService {
  ...
  async getCourse(id: string, actorPromise: Promise<Actor>) {
    ...
    if (!course || (course.status !== "published" && !this.canManage(actor))) throw new NotFoundException("课程不存在");
    const [lessons, resources, progress] = await Promise.all([
      this.db.query(`SELECT id, title, summary, lesson_type, sort_order, estimated_minutes, status FROM course_lessons WHERE course_id = $1 ORDER BY sort_order, created_at`, [id]),
      this.db.query(`SELECT id, lesson_id, title, resource_type, storage_key, external_url, file_name, mime_type, file_size, transcript_text, source_name, source_url, copyright_note, sort_order, status FROM course_resources WHERE course_id = $1 ORDER BY sort_order, created_at`, [id]),
  ...
  async updateProgress(actor: Actor, lessonId: string, input: ProgressInput) {
    await this.requireRow("course_lessons", lessonId, "课时不存在");
  ...
  async getWorkflow(id: string) {
    ...
    const [versions, tools] = await Promise.all([
      this.db.query(`SELECT id, version_number, definition_json, prompt_template, published_at FROM workflow_versions WHERE workflow_id = $1 ORDER BY version_number DESC`, [id]),
  ...
  async getWork(id: string, actorPromise: Promise<Actor>) {
    ...
    const [assets, workflows, comments] = await Promise.all([this.db.query(`SELECT * FROM work_assets WHERE work_id=$1 ORDER BY sort_order,created_at`, [id]), this.db.query(`SELECT w.id,w.name,w.entry_type,w.entry_url,ww.workflow_version_id FROM workflows w JOIN work_workflows ww ON ww.workflow_id=w.id WHERE ww.work_id=$1`, [id]), this.db.query(`SELECT c.id,c.content,c.parent_id,c.created_at,u.display_name AS author FROM comments c JOIN users u ON u.id=c.author_id WHERE c.work_id=$1 AND c.status='published' ORDER BY c.created_at`, [id])]);
  ...
  async updateWork(actor: Actor, id: string, input: Partial<WorkInput>) {
    ...
    if (fields.length) { values.push(id); await this.db.query(`UPDATE works SET ${fields.join(",")},updated_at=CURRENT_TIMESTAMP WHERE id=$${values.length}`, values); }
    return this.getWork(id,Promise.resolve(actor));
  }

  async listComments(workId: string) { await this.requireRow("works",workId,"作品不存在"); return { items: (await this.db.query(`SELECT c.id,c.content,c.parent_id,c.created_at,u.display_name AS author FROM comments c JOIN users u ON u.id=c.author_id WHERE c.work_id=$1 AND c.status='published' ORDER BY c.created_at`, [workId])).rows }; }
  async addComment(actor: Actor, workId: string, input: CommentInput) { await this.requireRow("works",workId,"作品不存在"); const result=await this.db.query(`INSERT INTO comments (id,work_id,author_id,parent_id,content) VALUES ($1,$2,$3,$4,$5) RETURNING *`,[randomUUID(),workId,actor.id,input.parentId ?? null,input.content]); return result.rows[0]; }

  async toggleReaction(actor: Actor, workId: string, kind: "like" | "favorite") { await this.requireRow("works",workId,"作品不存在"); const table=kind === "like" ? "work_likes" : "work_favorites"; const found=await this.db.query(`SELECT 1 FROM ${table} WHERE work_id=$1 AND user_id=$2`,[workId,actor.id]); if(found.rowCount){await this.db.query(`DELETE FROM ${table} WHERE work_id=$1 AND user_id=$2`,[workId,actor.id]);return {active:false};} await this.db.query(`INSERT INTO ${table} (work_id,user_id) VALUES ($1,$2)`,[workId,actor.id]); return {active:true}; }
  ...
  async createConversation(actor: Actor, input: ConversationInput) { const result=await this.db.query(`INSERT INTO conversations (id,user_id,course_id,workflow_id,conversation_type,title) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,[randomUUID(),actor.id,input.courseId ?? null,input.workflowId ?? null,input.type,input.title ?? null]); return result.rows[0]; }
